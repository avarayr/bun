...
pub const JSValkeyClient = struct {
    ...
    pub fn createNoJsNoPubsub(globalObject: *jsc.JSGlobalObject, arguments: []const JSValue) bun.JSError!*JSValkeyClient {
        ...
        // Track whether a URL was explicitly provided by the user
        const user_provided_url = arguments.len >= 1 and !arguments[0].isUndefined();

        const url_str = if (user_provided_url)
        ...
        // Only validate if a URL was explicitly provided (not when using defaults)
        ...
        const hostname_str = parsed_url.hostname();
