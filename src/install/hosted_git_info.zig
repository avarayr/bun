...
pub const HostedGitInfo = struct {
    ...
    fn moveFromExtracted(
        ...
        self.#memory_buffer = moved.buffer;
        self.#allocator = moved.allocator;
    ...
    pub fn fromUrl(allocator: std.mem.Allocator, git_url: []u8) !?Self {
        ...
        const pathname_owned = try parsed.url.sliceTools().pathname(allocator);
        ...
        const fragment = try parsed.url.sliceTools().fragmentIdentifier(allocator);
        ...
        const Formatters = struct {
            ...
            const Extract = struct {
                ...
                fn github(allocator: std.mem.Allocator, url: *jsc.URL) ?Result {
                    const pathname_owned = url.sliceTools().pathname(allocator) catch return null;
                    defer allocator.free(pathname_owned);
