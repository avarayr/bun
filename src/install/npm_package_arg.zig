...
pub const NpaSpec = struct {
    ...
    fn fromUrl(
        ...
        const protocol_slice = try parsed_url.sliceTools().protocol(allocator);
        ...
        var spec: NpaSpec = .{
            ...
            .name = undefined,
            .raw_spec = undefined,
            .#arena_buffer = undefined,
            .#fetch_spec_slice = undefined,
            .save_spec = undefined,
            .type = undefined,
            .#allocator = allocator,
        };
        ...
        switch (protocol_type) {
            .http, .https => {
                ...
                spec.#arena_buffer = arena.buffer;
                spec.#fetch_spec_slice = raw_spec_slice;
            ...
            => {
                ...
                spec.raw_spec = raw_spec_slice;
                spec.#arena_buffer = arena.buffer;
                spec.#fetch_spec_slice = fetch_spec_slice;
                spec.save_spec = raw_spec_slice;
                spec.type = .{
                    .git = .{
                        .attrs = git_attrs,
...
const GitAttrs = struct {
    ...
    pub fn fromUrl(allocator: std.mem.Allocator, parsed_url: *bun.jsc.URL) !?Self {
        const hash_slice = try parsed_url.sliceTools().hash(allocator);
...
const SpecStrUtils = struct {
    ...
    pub fn getUrlHrefWithoutHash(allocator: std.mem.Allocator, parsed_url: *bun.jsc.URL) ![]u8 {
        const href_slice = try parsed_url.sliceTools().href(allocator);
    ...
    pub fn extractHostAndPathnameWithLowercaseHost(
        ...
        const host_slice = try parsed_url.sliceTools().host(allocator);
        defer allocator.free(host_slice);
        const pathname_slice = try parsed_url.sliceTools().pathname(allocator);
